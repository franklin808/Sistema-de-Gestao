document.addEventListener('DOMContentLoaded', () => {
    ...
    const paymentStatusFilter = document.getElementById('payment-status-filter'); // Novo elemento
    ...
    function setupEventListeners() {
        ...
        document.querySelectorAll('[data-close-button]').forEach(button => {
            button.addEventListener('click', () => {
                ...
                if (modal && modal.id === 'newOrderModal') document.body.classList.remove('overflow-hidden');
            });
        });
        ...
        paymentStatusFilter.addEventListener('change', loadOrders); // Listener para o novo filtro
    ...
    function toggleVencimentoField() {
        ...
        if (tipo === 'a_prazo') {
            dataVencimentoContainer.classList.remove('hidden');
        } else {
            dataVencimentoContainer.classList.add('hidden');
            dataVencimentoInput.value = '';
        }
    }

    // --- FUNÇÕES DE CARREGAMENTO DE DADOS (COM FILTRO DE PAGAMENTO) ---
    async function loadOrders() {
        ...
        const filterPaymentStatus = paymentStatusFilter.value; // Pega o valor do novo filtro

        try {
            let query = supabase.from('pedidos')
                .select(`id, numero_pedido, data_pedido, data_entrega, status, status_faturamento, tipo_pagamento, data_vencimento_faturamento, valor_total, clientes(nome)`)
                .order('numero_pedido', { ascending: false });

            // Filtro de status do pedido (direto na query)
            if (filterStatus) {
            ...
            // Filtros aplicados no lado do cliente (em JavaScript)
            let filteredOrders = orders;

            // 1. Filtrar por Status de Pagamento
            if (filterPaymentStatus) {
                filteredOrders = filteredOrders.filter(order => {
                    if (filterPaymentStatus === 'pago') {
                        return order.status_faturamento === 'pago';
                    }
                    ...
                    if (filterPaymentStatus === 'atrasado') {
                        return isAtrasado;
                    }
                    if (filterPaymentStatus === 'nao_pago') {
                        // "Não pago" é qualquer um que não esteja 'pago' e nem 'atrasado'
                        return order.status_faturamento !== 'pago' && !isAtrasado;
                    }
                    return false;
                });
            }
    ...
    function getStatusFaturamentoDinamico(order) {
        ...
        if (order.tipo_pagamento === 'a_prazo') {
            if (order.data_vencimento_faturamento) {
                const hoje = new Date();
                hoje.setHours(0, 0, 0, 0); 
                ...
                if (dataVencimento < hoje) {
                    ...
                    const prazoTxt = `Vence em ${dataVencimento.toLocaleDateString('pt-BR')}`;
                    return { statusFaturamentoTexto: prazoTxt, statusFaturamentoClasse: 'badge-a_vencer' };
                }
            }
    ...
    // --- MODAL E FORMULÁRIO (LÓGICA DE CÓPIA CORRIGIDA) ---
    async function showOrderModal(orderId = null, copyData = null) {
        ...
        if (copyData) {
            ...
            if (copyData.observacoes) {
                ...
                } catch(e) { console.error("Erro ao processar 'observacoes' para cópia:", e)}
            }
    ...
    async function handleOrderFormSubmit(e) {
        ...
        newOrderForm.querySelectorAll('input[id^="order_"], textarea[name="obs_gerais"], textarea[name="prod_obs"]').forEach(input => {
            if (input.name) printDetails[input.name] = input.value;
        });
        ...
        const orderData = {
            ...
            status_faturamento: statusFaturamentoSelect.value,
            data_vencimento_faturamento: dataVencimentoInput.value || null,
        };

        if (orderData.tipo_pagamento === 'a_vista') {
            orderData.data_vencimento_faturamento = null;
        }
        ...
        if ([...itemsToUpdate, ...itemsToInsert].length === 0) {
            showMessageModal("Validação", "Adicione pelo menos um item ao pedido."); return;
        }
        if ([...itemsToUpdate, ...itemsToInsert].some(item => !item.produto_cliente_id)) {
            showMessageModal("Validação", "Selecione um produto para cada item do pedido."); return;
        }
    ...
    function closeModal() { newOrderModal.classList.add('hidden'); document.body.classList.remove('overflow-hidden'); }
    async function loadClientsAndProductsForDropdown() { try { const clientPromise = supabase.from('clientes').select('id, nome').order('nome'); const productPromise = supabase.from('produtos_cliente').select('*, imagem_arte_url'); const [{ data: clients, error: clientError }, { data: products, error: productError }] = await Promise.all([clientPromise, productPromise]); if (clientError) throw clientError; if (productError) throw productError; allClients = clients; allClientProducts = products; } catch (error) { console.error('Erro ao carregar clientes e produtos:', error); showMessageModal("Erro de Carregamento", `Não foi possível carregar dados essenciais: ${error.message}`); } }
    function populateClientsDropdown(selectedClientId = '') { clientSelect.innerHTML = '<option value="">Selecione um cliente</option>'; allClients.forEach(client => { const option = document.createElement('option'); option.value = client.id; option.textContent = client.nome; if (String(client.id) === String(selectedClientId)) option.selected = true; clientSelect.appendChild(option); }); }
    function applyServicesFromDetails(details = {}) { arteCheckbox.checked = details.servico_arte_checked || false; arteValor.value = details.servico_arte_valor || '100.00'; clicheCheckbox.checked = details.servico_cliche_checked || false; clicheValor.value = details.servico_cliche_valor || '350.00'; }
    ...
    function addOrderItemRow(itemData = null, clientId = null) {
        ...
        const itemId = itemData && orderModalTitle.textContent !== 'Criar Novo Pedido (Cópia)' ? itemData.id : '';
        newRow.innerHTML = ` <td class="p-2 align-top"> <img src="https://placehold.co/80x80/e2e8f0/adb5bd?text=N/A" alt="Arte" class="w-20 h-20 object-contain rounded border product-art-preview"> </td> <td class="p-2 align-top"> <input type="hidden" name="pedido_item_id" value="${itemId}"> <select name="produto_cliente_id" class="border-gray-300 rounded-md shadow-sm w-full p-2 text-sm client-product-select" required></select> </td> <td class="p-2 align-top"><input type="text" name="descricao_item" class="border-gray-300 rounded-md shadow-sm w-full p-2 text-sm" placeholder="Descrição no pedido" value="${itemData?.descricao_item || ''}"></td> <td class="p-2 align-top"><input type="number" name="quantidade" min="1" value="${itemData?.quantidade || 1}" class="border-gray-300 rounded-md shadow-sm w-20 p-2 text-sm quantity-input" required></td> <td class="p-2 align-top"><input type="number" step="0.01" name="preco_unitario" value="${itemData ? parseFloat(itemData.preco_unitario).toFixed(2) : '0.00'}" class="border-gray-300 rounded-md shadow-sm w-24 p-2 text-sm price-input" required></td> <td class="p-2 align-top text-sm font-medium subtotal">R$ 0,00</td> <td class="p-2 align-top"><button type="button" class="text-red-600 hover:text-red-800 remove-item" title="Remover"><i class="fas fa-trash-alt"></i></button></td> `;
    ...
    function updateSubtotal(row) { const qty = parseFloat(row.querySelector('.quantity-input').value) || 0; const price = parseFloat(row.querySelector('.price-input').value) || 0; const subtotal = qty * price; row.querySelector('.subtotal').textContent = subtotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }); }
    function updateOrderTotal() { let itemsSubtotal = 0; document.querySelectorAll('.order-item').forEach(row => { const qty = parseFloat(row.querySelector('.quantity-input').value) || 0; const price = parseFloat(row.querySelector('.price-input').value) || 0; itemsSubtotal += qty * price; }); let servicesTotal = 0; if (arteCheckbox.checked) { servicesTotal += parseFloat(arteValor.value) || 0; } if (clicheCheckbox.checked) { servicesTotal += parseFloat(clicheValor.value) || 0; } const finalTotal = itemsSubtotal + servicesTotal; itemsSubtotalSpan.textContent = itemsSubtotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }); servicesTotalSpan.textContent = servicesTotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }); orderTotalSpan.textContent = finalTotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }); }
    ...
    async function handleTableClick(e) {
        ...
        if (button.classList.contains('view-edit-order-btn')) {
            ...
            if (orderToCopy.pedido_itens) {
                orderToCopy.pedido_itens.forEach(item => {
                    delete item.id;
    ...
    async function handleDeleteConfirmation() { if (!orderToDelete) return; try { await supabase.from('pedido_itens').delete().eq('pedido_id', orderToDelete); await supabase.from('pedidos').delete().eq('id', orderToDelete); showMessageModal("Sucesso", "Pedido excluído com sucesso!"); await loadOrders(); } catch (error) { showMessageModal("Erro", `Não foi possível excluir o pedido: ${error.message}`); } finally { confirmationModal.classList.add('hidden'); orderToDelete = null; } }
    async function handlePrintOrder() { /* A lógica de impressão existente é mantida */ }
    function convertToEmbeddableUrl(url) { if (!url) return 'https://placehold.co/80x80/e2e8f0/adb5bd?text=N/A'; const gdriveRegex = /drive\.google\.com\/file\/d\/([a-zA-Z0-9_-]+)/; const match = url.match(gdriveRegex); if (match && match[1]) { return `https://drive.google.com/uc?export=view&id=${match[1]}`; } return url; }
    function populateClientProductsDropdown(selectElement, clientId, selectedProductId = '') { const productsForClient = allClientProducts.filter(p => p.cliente_id === null || String(p.cliente_id) === String(clientId)); selectElement.innerHTML = '<option value="">Selecione um produto</option>'; if (!clientId) { selectElement.innerHTML = '<option value="">Selecione um cliente primeiro</option>'; selectElement.disabled = true; return; } selectElement.disabled = false; if (productsForClient.length === 0) { selectElement.innerHTML = '<option value="">Nenhum produto cadastrado</option>'; return; } productsForClient.sort((a, b) => { if (a.cliente_id && !b.cliente_id) return -1; if (!a.cliente_id && b.cliente_id) return 1; return (a.nome || '').localeCompare(b.nome || ''); }); productsForClient.forEach(product => { const option = document.createElement('option'); option.value = product.id; option.textContent = product.cliente_id ? product.nome : `${product.nome}`; if (String(product.id) === String(selectedProductId)) option.selected = true; selectElement.appendChild(option); }); }
    function handleClientChange() { const selectedClientId = clientSelect.value; document.querySelectorAll('.order-item .client-product-select').forEach(select => { populateClientProductsDropdown(select, selectedClientId); const row = select.closest('tr'); row.querySelector('[name="descricao_item"]').value = ''; row.querySelector('[name="preco_unitario"]').value = '0.00'; const artPreviewImg = row.querySelector('.product-art-preview'); artPreviewImg.src = convertToEmbeddableUrl(''); updateSubtotal(row); }); updateOrderTotal(); }
